UPDATE core_config_data SET value = CONCAT('http://', SUBSTRING_INDEX(SUBSTRING_INDEX(value, 'http://', -1), '.', 1), '.quatrovets.com.br/')
WHERE path LIKE 'web/unsecure/base_url';

UPDATE core_config_data SET value =
case INSTR(value, 'https')
when true then CONCAT('http://', SUBSTRING_INDEX(SUBSTRING_INDEX(value, 'https://', -1), '.', 1), '.quatrovets.com.br/')
when false then CONCAT('http://', SUBSTRING_INDEX(SUBSTRING_INDEX(value, 'http://', -1), '.', 1), '.quatrovets.com.br/')
end
...
INSERT INTO `core_config_data` (scope, scope_id, path, value) VALUES ('default', '0', 'web/cookie/cookie_domain', '.quatrovets.com.br');
